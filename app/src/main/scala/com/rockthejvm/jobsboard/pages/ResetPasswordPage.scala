...
final case class ResetPasswordPage(
  ...
  override def update(msg: App.Msg): (Page, Cmd[IO, App.Msg]) = msg match {
    ...
    case ResetPasswordSuccess =>
      (setSuccessStatus("Success! You can login now"), Cmd.None)
...
object ResetPasswordPage {
  ...
  case class ResetPasswordFailure(str: String) extends Msg
    ...
    val resetPassword = new Endpoint[Msg] {
      ...
      override val onResponse: Response => Msg = response =>
        response.status match {
          ...
          case _ => ResetPasswordFailure("Unknown reply from server. Something's wrong!")
        }
    }
  }
